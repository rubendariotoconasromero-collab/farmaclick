...
use App\Models\Antiparasitario;
use App\Models\MiEmpresa;
use App\Models\DetalleAntiparasitario;
...
class AntiparasitarioController extends BitacoraController
{
    public function index(Request $request){
        $buscar = $request->buscar;
        $criterio = $request->criterio;
        if($buscar==''){
            $obj= Antiparasitario::join('paciente','antiparasitario.id_paciente','=','paciente.id')
            ->join('cliente','paciente.id_cliente','=','cliente.id')
            ->join('animal','paciente.id_animal','=','animal.id')
            ->join('detalle_antiparasitario','antiparasitario.id','=','detalle_antiparasitario.id_antiparasitario')
            ->select('antiparasitario.id','antiparasitario.id_paciente','antiparasitario.fecha','antiparasitario.prox_fecha','detalle_antiparasitario.peso',
            'cliente.nombre as cliente','animal.nombre as animal','paciente.nombre as paciente','paciente.id_animal','paciente.id_cliente','antiparasitario.sub_total','antiparasitario.total','paciente.edad')
            //->where('e.estado', 1)
            ->GroupBy('antiparasitario.id')
            ->orderBy('antiparasitario.id','desc')->paginate(15);
        }
        else{
            $obj= Antiparasitario::join('paciente','antiparasitario.id_paciente','=','paciente.id')
            ->join('cliente','paciente.id_cliente','=','cliente.id')
            ...
            ->join('detalle_antiparasitario','antiparasitario.id','=','detalle_antiparasitario.id_antiparasitario')
            ->select('antiparasitario.id','antiparasitario.id_paciente','antiparasitario.fecha','antiparasitario.prox_fecha','detalle_antiparasitario.peso',
            'cliente.nombre as cliente','animal.nombre as animal','paciente.nombre as paciente','paciente.id_animal','paciente.id_cliente','antiparasitario.sub_total','antiparasitario.total')
    ...
    public function guardar(Request $request){
        try{
                ...
                foreach($detalles as $ep=>$det){
                        $obj = new DetalleAntiparasitario();
                        $obj->id_antiparasitario= $control->id;
    ...
    public function VentaControlVacuna($request)
    {
                ...
                $venta->tipo_venta='Venta Antiparasitario';
    ...
    public function detalleAntiparasitario(Request $request){
        
        //dd($request->id);
        $id=$request->id; 
        $obj= DetalleAntiparasitario::join('tienda_articulo','detalle_antiparasitario.id_producto','=','tienda_articulo.id')
        ->join('articulo','tienda_articulo.id_articulo','=','articulo.id')
        ->join('categoria','articulo.id_categoria','=','categoria.id')
        ->select('detalle_antiparasitario.id_antiparasitario','articulo.nombre_comercial as articulo','detalle_antiparasitario.id_producto as id_tienda_articulo'
        ,'detalle_antiparasitario.id_producto as id_articulo','categoria.nombre as categoria','detalle_antiparasitario.costo_venta','articulo.costo_unitario',
        'articulo.costo_mayorista','articulo.costo_preferencial','articulo.nombre_comercial as articulo','articulo.id_categoria'
        ,'detalle_antiparasitario.cantidad','detalle_antiparasitario.sub_total','tienda_articulo.stock','detalle_antiparasitario.estado')
        ->where('detalle_antiparasitario.id_antiparasitario','=',$id)
    ...
    public function ActEstado(Request $request){
        $id=$request->id;  
        //dd($id);
        $affected = DB::table('detalle_antiparasitario')
    ...
    public function modificar(Request $request){
        try{
            ...
            $id= $request->id;
            //dd($id);
            $affected = DB::table('antiparasitario')
              ->where('id', $request->id)
            ...
            foreach($detalles as $ep=>$det){             
                if($det['estado'] ==0){
                $detall = DB::table('detalle_antiparasitario')->where('id_antiparasitario',$request->id)->where('id_producto',$det['id_tienda_articulo'])->first();
                // if ($detall == null) {
                    $obj = new detalleAntiparasitario();
                    $idDetalle = DB::table('detalle_antiparasitario')->max('id_antiparasitario');
                    $obj->id_antiparasitario= $request->id;
                    $obj->id_producto= $det['id_tienda_articulo'];
                    $obj->fecha=$request->fecha;
                    $obj->prox_fecha=$request->prox_fecha;
                    $obj->edad=$request->edad;
                ...
                $control= Antiparasitario::findOrFail($request->id);
                $control->fecha=$request->fecha;
                $control->prox_fecha=$request->prox_fecha;
                $control->save();
    ...
    public function pdfAntiparacitario(Request $request){

        $id = $request->id;

        $x=DB::select("SELECT detalle_antiparasitario.fecha,detalle_antiparasitario.peso,articulo.nombre_comercial as vacuna ,detalle_antiparasitario.prox_fecha
        FROM antiparasitario,detalle_antiparasitario ,tienda_articulo ,articulo	
        WHERE detalle_antiparasitario.id_antiparasitario=antiparasitario.id and detalle_antiparasitario.id_producto=tienda_articulo.id and tienda_articulo.id_articulo=articulo.id and antiparasitario.id='$id'");
        ...
        $cont=Antiparasitario::count();
        $pdf = \PDF::loadView('pdf.reportes.vacunas.antiparasitario', [
            'title'=>$title,
