    ...
    export default {
        ...
        methods : {
            ...
            listarVenta(page, buscar, criterio){
                let me=this;
                var url='/cotizacion3?page=' + page + '&buscar=' + buscar + '&criterio=' + criterio;
            ...
            guardarVenta(){
                ...
                if(this.arrayDetalle.find(seg => (seg.stock - seg.cantidad < 0 && this.datos.tipo_venta!='Venta Servicio'))){
                    ...
                    if(this.datos.total < 0){
                        ...
                        axios.post('/venta/guardar_tienda3/cotizacion',{
                            'id': me.datos.id,
            ...
            modificarCotizacion(){
                ...
                // } else {
                    if(this.datos.total < 0){
                        ...
                        let me = this;
                        axios.put('/cotizacion/modificar3',{
                            'id': me.datos.id,
                            'fecha': me.datos.fecha,
                            'fecha_venci': me.datos.fecha_venci,
                            'sub_total': me.datos.sub_total,
            ...
            cargarPdfSimple(id) {
                axios.get('/cotizacion/pdfCotizacionSimple3?id=' + id,{responseType: 'blob'})
                    .then(response => {
                        var blob = new Blob([response.data], {type: 'application/pdf'});
                        var downloadUrl = URL.createObjectURL(blob);
                        window.open(downloadUrl, '_blank');
