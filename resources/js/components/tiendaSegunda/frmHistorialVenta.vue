                        ...
                        <table class="table table-striped table-hover" style='width:96%;margin-left: 2.2%'>
                            ...
                            <tbody>
                                <tr v-for="cotizacion in arrayVentaCotizacion" :key="cotizacion.id">
                                    ...
                                    <!-- <td>
                                        <button type="button" class="btn btn-info text-white position-relative" data-bs-toggle="modal" data-bs-target="#modalPago" :disabled="cotizacion.tipoP == 'Contado' || cotizacion.estado == 'Anulado' ? true : false" @click="realizarPagos(cotizacion)">Pagos</button>    
                                    </td>  -->  
                                    <!-- <td>
                                        <button class="btn btn-outline-secondary dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">Accion</button>
                                        <ul class="dropdown-menu dropdown-menu-end">
    ...
    export default {
        ...
        methods : {
            listarVenta(page, buscar, criterio){
                let me=this;
                var url='/venta_tienda2?page=' + page + '&buscar=' + buscar + '&criterio=' + criterio;
            ...
            listarVentaBusquedaRapida(){
                let me=this;
                var url='/venta_tienda2?page=' + 1 + '&buscar=' + me.buscar + '&criterio=' + me.criterio;
                axios.get(url).then(function(response){
                    me.arrayVenta=response.data.data;
                    me.pagination={total:response.data.total, 
                        current_page:response.data.current_page,
            ...
            listarVentaServicio(page, buscar, criterio){
                let me=this;
                var url='/ventaServicio_tienda2?page=' + page + '&buscar=' + buscar + '&criterio=' + criterio;
                axios.get(url).then(function(response){
            ...
            listarVentaCotizacion(page, buscar, criterio){
                let me=this;
                var url='/ventaCotizacion_tienda2?page=' + page + '&buscar=' + buscar + '&criterio=' + criterio;
                axios.get(url).then(function(response){
            ...
            listarVentaServicioVentaDirecta(){
                let me=this;
                var url='/ventaServicio_tienda2?page=' + 1 + '&buscar=' + me.buscar + '&criterio=' + me.criterio;
                axios.get(url).then(function(response){
                    me.arrayVentaServicio=response.data.data;
                    me.paginationServicio={total:response.data.total, 
                        current_page:response.data.current_page,
            ...
            verVenta(data=[]){
                ...
                var url='/venta/permiso/detalle_tienda2?id=' + data['id'];
                axios.get(url).then(function(response){
            ...
            anularVenta(id,id_servicio){
                ...
                if (result.isConfirmed) {
                    let me = this;
                    axios.put('/venta/anular_tienda2',{'id': id,'id_orden_servicio': id_servicio}).then(function (response) {
                        me.listarVenta(1,'', 'nombre');
