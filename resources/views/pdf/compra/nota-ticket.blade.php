...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .notes { margin-top: 7px; padding-top: 6px; border-top: 1px dashed #555; }
...
<body>
    @include('pdf.compra.partials.brand-header', ['compact' => true])
    ...
    @include('pdf.compra.partials.purchase-meta', ['compact' => true])
        ...
        <tbody>
            @foreach($detalles as $detalle)
                <tr><td>
                    ...
                    <div class="item__meta">Lote: {{ $detalle->lote ?: 'S/L' }} | Vence: {{ $detalle->fecha_vecimiento ? \Carbon\Carbon::parse($detalle->fecha_vecimiento)->format('d/m/Y') : 'S/F' }}</div>
                    <div class="item__calc">
                        {{ number_format((float) $detalle->cantidad, 0, ',', '.') }} x Bs {{ number_format((float) $detalle->costo_compra, 2, ',', '.') }}
                        @if((float) $detalle->descuento > 0) - Desc. Bs {{ number_format((float) $detalle->descuento, 2, ',', '.') }} @endif
                        <span class="item__subtotal">Bs {{ number_format((float) $detalle->sub_total, 2, ',', '.') }}</span>
                    </div>
                </td></tr>
            @endforeach
        </tbody>
    ...
    @include('pdf.compra.partials.purchase-totals')

    @if($compra->descripcion)
        <div class="notes"><strong>Observaciones</strong>{{ $compra->descripcion }}</div>
    @endif

    <div class="footer">
        <strong>Gracias por registrar su compra</strong>
        Documento de control interno<br>
        <small>FarmaClick - {{ $fecha_impresion }}</small>
    </div>
</body>
</html>
