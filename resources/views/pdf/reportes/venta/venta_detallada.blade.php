...
<head>
    ...
    <title>{{ $title ?? 'Reporte de Ventas' }}</title>
</head>
@php
    $colors = [
        'color1' => '#001843',
            ...
            <thead>
                <tr>
                    <th style="text-align: center; padding: 25px; background-color: #f8f8f8; color: #666; font-weight: normal;">
                        No se encontraron registros entre las fechas {{ \Carbon\Carbon::parse($fecha_inicio)->format('d/m/Y') ?? 'N/A' }} y {{ \Carbon\Carbon::parse($fecha_fin)->format('d/m/Y') ?? 'N/A' }}
                    </th>
                </tr>
            </thead>
        </table>
