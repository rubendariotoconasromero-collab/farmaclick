...
<head>
    ...
    <title>{{ $title ?? 'Reporte de Ventas Anuladas' }}</title>
</head>
@php
    $colors = [
        'color1' => '#001843',
...
<body>
    ...
    <table class="table-head" style="width: 100%; border-bottom: 1px solid {{ $colors['color1'] }}; margin-bottom: 20px;">
        <tr>
            <!-- Logo de la empresa -->
            <th rowspan="3" style="width: 10%; text-align: center; vertical-align: middle; padding: 5px;" class="no-border">
                @if($foto_empresa)
                    <img src="{{ 'img/logo/' . $foto_empresa }}" height="60" alt="Company Logo">
                @else
        ...
        <thead>
            <tr>
                <th style="background-color: {{ $colors['color1'] }}; color: #FFF; padding: 10px; text-align: left;">
                    TOTAL VENTAS ANULADAS: {{ number_format($totales->totalV ?? 0, 2) }} Bs.
                </th>
            </tr>
            <tr>
            ...
            <thead>
                <tr>
                    <th style="text-align: center; padding: 25px; background-color: #f8f8f8; color: #666; font-weight: normal;">
                        No se encontraron ventas anuladas entre las fechas {{ \Carbon\Carbon::parse($fecha_inicio)->format('d/m/Y') ?? 'N/A' }} y {{ \Carbon\Carbon::parse($fecha_fin)->format('d/m/Y') ?? 'N/A' }}
                    </th>
                </tr>
            </thead>
        </table>
