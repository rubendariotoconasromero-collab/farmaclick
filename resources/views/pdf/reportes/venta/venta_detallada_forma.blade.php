@php
    $eyebrow = 'Ventas';
    $documentLabel = 'Ventas por forma de pago';
    $sectionTitle = $title ?? 'Ventas detalladas';
    $description = 'Detalle de ventas y productos filtrados por forma de pago, dentro del arqueo de caja.';
    $recordCount = isset($venta) ? $venta->count() : 0;
    $recordLabel = 'Ventas';
    $footerLabel = 'Ventas por forma de pago';
@endphp
    ...
    <tbody>
        @forelse(($venta ?? collect()) as $comp)
            @php
                ...
                $totalMostrar = ($comp->forma_pago ?? '') == 'Efectivo'
                    ? ($comp->total_efectivo ?? 0)
                    : ($comp->total_deposito ?? $comp->total ?? 0);
            @endphp
            <tr class="fc-group-row">
                <td colspan="5">
                    {{ $comp->cliente ?? 'N/A' }} · {{ $comp->tipo_pago ?? 'N/A' }} / {{ $comp->forma_pago ?? 'N/A' }}
                    <div class="is-muted">Descuento: Bs {{ number_format((float) ($comp->descuento ?? 0), 2) }} · Total: Bs {{ number_format((float) $totalMostrar, 2) }}</div>
                </td>
            </tr>
