...
<body>
    ...
    <div class="container">
        ...
        <table width="100%">
            <tr width="100%">
                <td width="100%" style="text-align: center" colspan="6">
                    <strong><u><FONT FACE="Cambria" style="color: black" SIZE=5>LISTA DETALLADA DE PROFORMAS</FONT></u></strong>
                </td>
            </tr>
            <tr width="100%" >
        ...
        @foreach($proforma as $ord)
            <table class="table" style="">
                <thead style="">
                    <tr> 
                    <td style="border: 1px solid black; background-color:<?php echo $color1; ?> ;text-align: left; border-right: 1px solid black;width:13%"><strong>{{ 'Código:' }} {{ $ord['codigo_proforma'] }}</strong></td>
                        <td style="border: 1px solid black; background-color:<?php echo $color1; ?> ;text-align: left; border-right: 1px solid black;width:13%"><strong>{{ 'Descuento:' }} {{ $ord['descuento'] }}</strong></td>
                        <td style="border: 1px solid black; background-color:<?php echo $color1; ?> ;text-align: left; border-right: 1px solid black;width:14%"><strong>{{ 'Total:' }} {{ $ord['total'] }}</strong></td>
                        <td style="border: 1px solid black; background-color:#DCE6F1 ;text-align: center; border-right: 1px solid black;width:20%"><strong>{{ 'Cantidad' }} </strong></td>
                        <td style="border: 1px solid black; background-color:#DCE6F1 ;text-align: center; border-right: 1px solid black;width:20%"><strong>{{ 'PU' }} </strong></td>
                        <td style="border: 1px solid black; background-color:#DCE6F1 ;text-align: left; border-right: 1px solid black;width:20%"><strong>{{ 'Sub Total:' }}{{ ' ' }}{{ $ord['sub_total'] }}{{ ' ' }}{{ 'Bs' }} </strong></td>
                    </tr>
                </thead>
                    
                @foreach($detalles as $det)
                    @if($det['id_proforma'] === $ord['id'])

                        <tbody  style="border-left: 1px solid black; border-right: 1px solid black; border-bottom: 1px solid black">
                        
                        <tr style="border-top: 1px solid black">
                            <td colspan="3" style="text-align: center; border-right: 1px solid black;width:40%">{{ $det['producto'] }}</td>
                            <td style="text-align: center; border-right: 1px solid black;width:20%">{{ $det['cantidad'] }}</td>
                            <td style="text-align: center; border-right: 1px solid black;width:20%">{{ $det['pu']}}</td>
                            <td style="text-align: center; border-right: 1px solid black;width:20%">{{ $det['sub_total']}}</td>
                        </tr>
                        </tbody>
